...
**Your answer:**
each facet's x-axis stretches to fit its own largest bar, so smaller regions aren't visually squished next to larger ones, while the actual counts remain readable on the axis. So a region with a max count of 50 fills its panel just as fully as a region with a max of 500. This keeps bars alligned but also keeps the count of opinions understandable.
...
**Original plot emphasizes:** The scale of X between the regions as larger and more spread apart, so scale differences between regions

**Free scales plot emphasizes:** The regions seem more evenly spread in terms of their answers, opinion distribution within each region
...
**Your answer:** 
To me free_X visualization would be misleading if we are only comparing the absolute number of responses across regions, it can make it seem that regions responses do not differ as much as they do. A bar spanning the full width of a small region's panel looks identical to a bar spanning the full width of a large region's panel
...
**Original plot (counts) shows:** Original plot shows a strict count of each opinion in each region, making larger regions visually dominate and smaller regions harder to interpret.


**Proportion plot shows:** Proportion show the share of opinion within each of the regions, which helps put all regions on equal footing regardless of size of the region and ammount of samples they were able to get.


**Which regions have the most "Wrong" responses as a proportion?** Scotland and London
...
**Faceted plot makes it easier to:** read the opinion breakdown within each region, since each region has its own dedicated panel with no visual clutter from other regions.

**Dodged plot makes it easier to:** compare the same opinion across regions side by side so we can see if each region has more dot knows or opinions compared to other regions easier.

**Task 3.3:** Which region shows the strongest opinion one way or the other (least "Don't know")?

**Your answer:** Scotland
...
I would use the faceted proportion plot because it would be easier to read if Scotland's "Wrong" proportion is towering relative to other opinions within that region, and places all regions on equal footing so the argument isn't inflated by sample size differences.
___________________________________________________________________________
___________________________________________________________________________
...
**Your answer:**
the original count plot with fixed scales because the shared x-axis lets you directly compare how many people responded in each region.
___________________________________________________________________________
___________________________________________________________________________
...
**Your answer:**
The same underlying data can tell completely different stories depending on whether you show counts or proportions, fixed or free scales, facets or dodged bars. So every choice you make while generating visuals is also a framing choice and highlights specific comparisons while making it harder to read other comparisons, it depends on what you are creating a chart to show, and what you are presenting.
___________________________________________________________________________
___________________________________________________________________________
___________________________________________________________________________
...
```{r wrong-only, eval=TRUE}
brexit_prop %>%
  filter(opinion == "Wrong") %>%
  ggplot(aes(x = reorder(region, prop), y = prop, fill = prop)) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Proportion who said Brexit was Wrong, by region",
    subtitle = "YouGov Survey Results, 2-3 September 2019",
    caption = "Source: bit.ly/2lCJZVg",
    x = NULL,
    y = "Proportion saying Wrong"
  ) +
  scale_y_continuous(labels = percent_format()) +
  scale_fill_gradient(low = "#f7c6b0", high = "#ef8a62") +
  guides(fill = "none") +
  theme_minimal()
```
...
2. ✅ Change `eval=TRUE` to `eval=TRUE` for all exercises
