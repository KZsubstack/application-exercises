...
# STEM laureates
nobel_stem <- nobel %>%
  filter(category %in% c("Medicine", "Physics", "Chemistry"))
```
...
nobel_non_stem <- nobel %>%
  filter(category %in% c("Literature", "Economic Sciences", "Peace"))
```
...
**Does the number of rows match what you expected?** yes 607 matches our previous nrow query on stem laureates
...
```{r write-summary}
write_csv(category_gender_summary, "data/nobel-summary.csv")
```
...
I learned that we can write filters of datasets into csvs that help optimize our workflow and branch out of the original dataset by having datasets tied to specific filters or categories.
...
Splitting data into subsets allows us to write less queries and reuse datasets that we've generated before and manipulate these subsets further a lot easier.
