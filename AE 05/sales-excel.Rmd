...
# Fill in the blanks
sales <- read_excel(
  "data-raw/sales.xlsx",           # File path
  skip = 2 # Number of rows to skip (try 0, 1, 2, 3 and see what works)
)
...
**Your answer:**
Each row represents a label or an individual item sold under that brand.

___________________________________________________________________________

**Task 2.2:** What does each column represent?
Each Column represents a brand and the number sold of that item.
___________________________________________________________________________
...
**Your answer:**
No, this data is not in tidy format. Brand names like Brand 1 and Brand 2 are mixed into the data rows instead of being stored in their own separate column, and one column contains two different types of values, both brand names and item IDs. In a tidy dataset, every column should represent one variable and every row should represent one observation of the same type, which is not the case here.
___________________________________________________________________________
___________________________________________________________________________
...
# Fill in the blanks
sales_tidy <- sales %>% # had to figure out a different method because I didn't get how to answer this with only pivoting
  rename(id = `Brand 1`) %>%
  mutate(brand = if_else(str_detect(id, "Brand"), id, NA_character_)) %>%
  fill(brand) %>%
  mutate(brand = replace_na(brand, "Brand 1")) %>%  
...
Original: 2 brands × 3-4 years = 7 rows in tidy format
