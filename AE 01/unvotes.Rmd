...
**Answer:** The issue categories are: Human rights,	Economic development, Colonialism, Palestinian conflict, Arms control and disarmament, and Nuclear weapons and nuclear material.
---
...
**Your answer:** The overall trend for the United Kingdom on Human Rights issues begins at around 1945 at a level of roughly 30% and remains pretty steady through the 50s and 60s. During the 1970s and early 1980s, the percentage of “Yes” votes declines very slightly, dipping to around 27-29%. From the mid 1980s, the trend gradually increases with a bit of momentum. Throughout the 1990s, the UK’s “Yes” vote percentage rises quite a bit, reaching its peak in the early 2000s at approximately 50–55%. After this peak, the percentage begins to decline somewhat during the 2010s, but it does not fall as low as any time between 1945-1990s. By the late 2010s, it ends up settling at around 37–39%.
...
**Your answer:** The United Kingdom appears most consistent because its voting percentages stay within a smaller range and change slowly over time. The line is more steady compared to the others. The United States appears least consistent because its voting shows sharp drops and big swings across several issues, especially in colonialism and the Palestinian conflict.
...
Most similar: Arms Control and Disarmament because their voting percentages stay relatively close and follow similar trends over time. 
...
unvotes %>%
  filter(country %in% c("Canada", "Mexico", "China")) %>%  # Change these!
  mutate(year = year(date)) %>%
  group_by(country, year, issue) %>%
  summarize(percent_yes = mean(vote == "yes")) %>%
  ggplot(mapping = aes(x = year, y = percent_yes, color = country)) +
...
**Which countries did you choose?** Canada, Mexico, and China

**Task 3.2:** Describe one interesting pattern you notice in your new plot.

**Your answer:** One interesting pattern I see right away is that China's data starts later than  Canada, Mexico, USA, Turkey, and UKs. On a lot of these issues China's percentage of yes is on the higher end and especially for the palestinian conflict and colonialism china's answers were very steady. Canada’s percentages are more in the middle and show more ups and downs, especially on the Palestinian conflict, where its support drops sharply in later years like the US. Overall, China and Mexico appear more aligned with each other on many issues, and mexico has pretty sharp rises at times when approaching the 80s. Canada shows more variation over time. 
...
unvotes %>%
  filter(country %in% c("Canada", "Mexico", "China", "United States", "United Kingdom")) %>%  # Choose 5 countries
  ...
  scale_y_continuous(labels = percent) +
  labs(
    title = "% of Yes votes for Human Rights",  # Update title
    subtitle = "1946 to 2019",
    y = "% Yes",
    x = "Year",
    color = "Country"
...
I see that the North American nations, Canada UK and US are very alligned and have similar answers to Human rights votes they hover generally within the range of 25% to 50% and follow mostly a similar shape besides US being more steadily flat from 1990-2020s. Mexico and China tend to scatter more towards the higher end.
---
...
**Which has the lowest?** Zanzibar has the lowest percentage of “yes” votes at 0%. However, this could just be due to a very small number of total votes in the dataset which is only 1, which could make the percentage not representative. For next country with a more substantial sample votes, the United States has the lowest percentages at 20.71%.
...
agreement_data <- unvotes %>%
  filter(country %in% c("United States", "China"),
         issue == "Human rights") %>%
  mutate(year = year(date)) %>%
  select(rcid, country, vote, year) %>%
  pivot_wider(names_from = country, values_from = vote) %>%
  filter(!is.na(`United States`) & !is.na(China)) %>%
  mutate(agree = (`United States` == China))
agreement_yearly <- agreement_data %>%
  group_by(year) %>%
  summarize(percent_agreement = mean(agree) * 100)
ggplot(agreement_yearly, aes(x = year, y = percent_agreement)) +
  geom_line(color = "blue") +
  labs(
    title = "US-China Voting Agreement on Human Rights",
    y = "Percent of Agreement",
    x = "Year"
  ) +
  theme_minimal()
