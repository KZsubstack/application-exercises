...
**How many movies passed the test?** 753
...
**Your answer:** One possible explanation is that budget and domestic gross are positively correlated. Movies with larger budgets often spend more on production quality, marketing, and distribution, which can increase their domestic earnings. If movies that fail the Bechdel test tend to receive higher budgets, this may partly explain why they also show higher median domestic gross. 

This isn't related to the answer, but one possible hypothesis is that movies that fail the Bechdel test may be more likely to be directed or produced by men. If male led projects historically receive larger budgets due to structural biases or existing power dynamics in the film industry, this could contribute to the observed budget gap.
...
**Your answer:** Paranormal Activity

**Task 4.2:** What is its ROI value?

**Your answer:** 671.336857

**Task 4.3:** Does this movie surprise you? Why or why not?

**Your answer:**It surprises me because I've never heard of Paranormal Activity in my life. After doing a Google search the movie got decent reviews and its budget was 15,000 that's very small compared to modern movies costing millions or billions. 
...
**Your answer:**There are movies that have super high roi compared to the rest in their Detailed bechdel category and also, this also compresses the other smaller more clustered data and then the difference between the majority of movies are going to be very difficult to see.
...
**Your answer:** These budgets vary a lot but they are all on the lower end. The higher ROI movies all have small budgets. They look like they are low budget films that earned very large domestic grosses relative to their production costs. ROI values that are high tend to come from inexpensive films that perform well at the box office compared to how much was spent.
...
ggplot(data = bechdel90_13, 
  ...
  labs(
    title = "Return on investment vs. Bechdel test result",
    subtitle = "Zoomed in the Y level to < 15 for better visibility on the more moderate trends", # Something about zooming in to a certain level
    x = "Detailed Bechdel result",
    y = "Return on investment",
    color = "Binary Bechdel result"
  ) +
...
**Your answer:** the OK category where Bechdel test passess

**Task 7.2:** Do movies that pass the Bechdel test (binary = PASS) appear to have higher or lower ROI than those that fail?

**Your answer:**

On average movies that pass the Bechdel test have a higher ROI than those who fail

**Task 7.3:** What does this suggest about the financial argument for including women in films?

**Your answer:** This suggests that a moving assing the Bechdel test does not effect it's roi negatively and including women in films who talk about things that are not men doesn't hurt any roi and doing this could also be associated with higher roi. The data Does NOT support an argument that films with good female representation end up perform worse.
...
**Mean ROI for PASS:** 7.994707

**Mean ROI for FAIL:** 8.362388

**Task 8.2:** How do these means compare to the medians we saw earlier? Why might they be different?

**Your answer:** The difference between what we saw earlier on the graph could be the fact that the mean is more sensitive to extreme values. we are looking at more fail categories and some fail movies have high roi outliers so the extreme values pull the mean upward, the median isn't effected as much by outliers and reflects the typical movie in each of the separate categories.
...
bechdel90_13 %>%
  ...
  ggplot(aes(x = year, y = mean_roi, color = binary)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Mean ROI Over Time by Bechdel Test Result",
    x = "Year",
    y = "Mean ROI",
    color = "Bechdel Result"
  )
```

**What trend do you notice?**There are a lot of variability in ROI that are influenced by very high performing films and there isn't a  pattern that show either pass or fail movies generate higher returns over time generally.
